...
| `.gitignore` | 忽略 `node_modules/`、`dist/`、`.astro/`、`.env`、`/src/generated/prisma` |
| Git Remote | `origin` → `https://github.com/idollive/my-blog.git`（master 分支） |
| `.env` | ✅ 步骤 3 创建 | 环境变量。`DATABASE_URL`（PlanetScale MySQL 连接字符串占位符）+ `ADMIN_PASSWORD_HASH`（管理员密码哈希占位符）。已被 `.gitignore` 排除，不进入版本控制 |
| `prisma.config.ts` | ✅ 步骤 3 创建 | Prisma 6 运行时配置文件。声明 schema 路径、migrations 目录、engine 类型（`classic`，兼容 PlanetScale）、datasource URL（从 `.env` 读取）。仅被 Prisma CLI 使用，不在 Astro 运行时加载 |

### `prisma/` 目录

| 文件 | 作用 |
|------|------|
| `prisma/schema.prisma` | ✅ 步骤 3 创建 | Prisma 数据模型定义文件。包含三层配置：**generator client**（输出到 `src/generated/prisma`，类型安全的查询客户端）、**datasource db**（provider=`mysql`，URL 从 `.env` 读取）、**models**（Post/Category/Tag/PostTag，步骤 6 定义）。Prisma CLI 将此文件作为数据库 schema 的唯一数据源 |
...
| `src/generated/prisma/` | 🔜 步骤 8 | Prisma Client 生成目录（`prisma generate` 输出）。不手动编辑，由 Prisma CLI 自动生成类型安全的数据库客户端代码 |
